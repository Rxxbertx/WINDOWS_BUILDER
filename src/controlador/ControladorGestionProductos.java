...
public class ControladorGestionProductos implements ActionListener, ItemListener, ChangeListener {
	...
	/**
	 * Controlador para la gestión de productos en la interfaz gráfica. Se encarga
	 * de controlar las interacciones entre la ventana principal y la ventana de
	 * gestión de productos.
	 */
	public ControladorGestionProductos(VentanaPrincipal ventana) {

		// Se crea una nueva instancia de la ventana de gestión de productos y el modelo
		// de producto
		...
		// se añaden listeners a los textField

		ventanaProducto.getTextFieldStock().addActionListener(this);
		...
		ventanaProducto.getComboBoxCategoriaConsulta().addItemListener(this);
		...
		ventanaProducto.getComboBoxCategoriaConsulta()
				.setModel(new DefaultComboBoxModel<>(modeloProducto.getCategorias()));
	...
	private void accionesPanelBaja(ActionEvent e) {
		...
		if (ventanaProducto.getBtnModificar().equals(e.getSource())) {
			...
			if (ventanaProducto.getComboBoxProductos().getSelectedIndex() != -1) {
				...
				try {
					// Se actualizan los datos del producto con los valores ingresados en la
					// interfaz
	...
	private void accionesPanelConsulta(ActionEvent e) {
		...
		if (ventanaProducto.getTextFieldStock() == (e.getSource())) {

			System.out.println(ventanaProducto.getTextFieldStock().getValue());

			eliminarDatosTabla();
			añadirDatosTabla(Integer.parseInt(ventanaProducto.getTextFieldStock().getValue().toString()));
	...
	private void accionesPanelAlta(ActionEvent e) {
		...
		if (ventanaProducto.getBtnGuardar().equals(e.getSource())) {
			...
			try {
				...
				// Se agrega el nombre del producto a la lista de nombres de productos en la
				// interfaz
	...
	/**
	 * Limpia los campos de texto y selecciones de la interfaz, según la opción
	 * especificada.
	 * 
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (e.getSource().equals(ventanaProducto.getComboBoxProductos())) {
			...
			if (ventanaProducto.getComboBoxProductos().getSelectedIndex() != -1) {
				...
				// Actualizar los campos de texto y selecciones en el panel de baja con los
				// datos del producto
		...
		if (e.getSource().equals(ventanaProducto.getComboBoxCategoriaConsulta())) {
			if (ventanaProducto.getComboBoxCategoriaConsulta().getSelectedIndex() != -1) {
				eliminarDatosTabla();
				añadirDatosTabla(ventanaProducto.getComboBoxCategoriaConsulta().getSelectedItem().toString());
	...
	private void eliminarDatosTabla() {
		...
		int rowCount = ventanaProducto.getModelTabla().getRowCount();
		for (int i = rowCount - 1; i >= 0; i--) {
			ventanaProducto.getModelTabla().removeRow(i);

		}
	}

	private void añadirDatosTabla(String string) {

		for (Map.Entry<Integer, Producto> entry : modeloProducto.getAlmacenProductos().entrySet()) {
			Integer key = entry.getKey();
			Producto val = entry.getValue();
			if (val.getCategoria().equals(string)) {
				ventanaProducto.getModelTabla()
						.addRow(new String[] { key.toString(), val.getNombre(), String.valueOf(val.getCantidad()),
								val.getCategoria(), String.valueOf(val.getPrecio()) + " €" });

			}

		}

		int preferredWidth = 100; // Ancho preferido para todas las columnas

		ventanaProducto.getTable().getColumnModel().getColumn(1).setPreferredWidth(preferredWidth);
		ventanaProducto.getTable().getColumnModel().getColumn(3).setPreferredWidth(preferredWidth);

	}

	private void añadirDatosTabla(int cantidad) {

		for (Map.Entry<Integer, Producto> entry : modeloProducto.getAlmacenProductos().entrySet()) {
			Integer key = entry.getKey();
			Producto val = entry.getValue();
			if (val.getCantidad() < (cantidad)) {
				ventanaProducto.getModelTabla()
						.addRow(new String[] { key.toString(), val.getNombre(), String.valueOf(val.getCantidad()),
								val.getCategoria(), String.valueOf(val.getPrecio()) + " €" });
	...
	/**
	 * Método que se ejecuta cuando se produce un cambio de estado en los
	 * componentes que implementan la interfaz ChangeListener.
	 * 
	...
	public void stateChanged(ChangeEvent e) {
		if (e.getSource().equals(ventanaProducto.getRdbtnCategoria())) {
			if (ventanaProducto.getRdbtnCategoria().isSelected()) {
				...
				ventanaProducto.getModelTabla().setColumnIdentifiers(modeloProducto.getCabeceraTabla());

				ventanaProducto.getLblCategoria3().setVisible(true);
				ventanaProducto.getComboBoxCategoriaConsulta().setVisible(true);

				ventanaProducto.getLblStock().setVisible(false);
				ventanaProducto.getTextFieldStock().setVisible(false);

				try {
					ventanaProducto.getComboBoxCategoriaConsulta().setSelectedIndex(0);
					añadirDatosTabla(ventanaProducto.getComboBoxCategoriaConsulta().getSelectedItem().toString());
				} catch (Exception e2) {
					// TODO: handle exception
				}
		...
		if (e.getSource().equals(ventanaProducto.getRdbtnStock())) {
			if (ventanaProducto.getRdbtnStock().isSelected()) {
				// Acciones a realizar cuando se selecciona la opción de categoría
				eliminarDatosTabla();
				...
				ventanaProducto.getLblCategoria3().setVisible(false);
				ventanaProducto.getComboBoxCategoriaConsulta().setVisible(false);

				ventanaProducto.getLblStock().setVisible(true);
				ventanaProducto.getTextFieldStock().setVisible(true);

				ventanaProducto.getTextFieldStock().setValue(0);
